...
// Aggregate deltoid heads into a single entry for charts (trainer view),
// while BodyMap keeps the per-head detail.
function aggregateDeltoids(volume: VolumeByMuscleGroup[]): VolumeByMuscleGroup[] {
  const deltoidKeys = new Set<VolumeByMuscleGroup['muscleGroup']>([
    'anterior_deltoid',
    'lateral_deltoid',
    'posterior_deltoid',
  ]);
  const deltoids = volume.filter((v) => deltoidKeys.has(v.muscleGroup));
  const others = volume.filter((v) => !deltoidKeys.has(v.muscleGroup));

  if (deltoids.length === 0) return volume;

  const sumEffectiveReps = deltoids.reduce((s, v) => s + v.effectiveReps, 0);
  const sumEffectiveSets = deltoids.reduce((s, v) => s + v.effectiveSets, 0);
  const sumTotalSets = deltoids.reduce((s, v) => s + v.totalSets, 0);
  const weightedStress = deltoids.reduce((s, v) => s + v.stressIndex * v.totalSets, 0);
  const avgStress = sumTotalSets > 0 ? weightedStress / sumTotalSets : deltoids.reduce((s, v) => s + v.stressIndex, 0);

  const aggregated: VolumeByMuscleGroup = {
    muscleGroup: 'posterior_deltoid', // thresholds closer to lateral/posterior work
    label: 'Deltoides',
    effectiveReps: Math.round(sumEffectiveReps),
    effectiveSets: Math.round(sumEffectiveSets * 10) / 10,
    totalSets: Math.round(sumTotalSets * 10) / 10,
    stressIndex: Math.round(avgStress * 10) / 10,
  };
...
export function MicrocycleCharts({ weekNumber, trainingDays, exercises }: MicrocycleChartsProps) {
  ...
  // Trainer view: deltoids aggregated; BodyMap will keep per-head detail
  const volumeForCharts = useMemo(
    () => aggregateDeltoids(metrics.volumeByMuscleGroup),
    [metrics.volumeByMuscleGroup]
  );

  // Data for Effective Sets by Muscle Group chart (RP Volume Landmarks)
  const setsBarData = useMemo(() => volumeForCharts.map((item) => {
  ...
  const repsBarData = useMemo(() => volumeForCharts.map((item) => {
