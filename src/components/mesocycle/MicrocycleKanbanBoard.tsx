...
interface MicrocycleKanbanBoardProps {
  ...
  onCloneTrainingDay?: (sourceDayId: string, insertionIndex: number) => Promise<void>;
...
interface ActiveDragItem {
  type: 'library-exercise' | 'day-exercise' | 'training-day';
  exercise?: Exercise;
...
export function MicrocycleKanbanBoard({
  ...
  const { warmupCount, mainCount, cooldownCount, restDayCount } = useMemo(() => {
    let warmup = 0;
    let main = 0;
    let cooldown = 0;
    let rest = 0;
    ...
    trainingDays.forEach((day) => {
      ...
      day.exercises?.forEach((ex) => {
        if (ex.phase === 'warmup') warmup += 1;
        else if (ex.phase === 'cooldown') cooldown += 1;
        else main += 1;
      });
    });

    return { warmupCount: warmup, mainCount: main, cooldownCount: cooldown, restDayCount: rest };
  }, [trainingDays]);
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    ...
    if (id.startsWith('training-day-')) {
      const dayId = id.replace('training-day-', '');
      const day = trainingDays.find((d) => d.id === dayId);
      if (day) {
        setActiveItem({ type: 'training-day', trainingDay: day });
      }
      return;
    }
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    if (activeId.startsWith('training-day-')) {
      ...
      if (overId.startsWith('day-')) {
        ...
        targetDayId = result?.day.id || null;
      }
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    if (activeId.startsWith('training-day-')) {
      ...
      const targetIndex = sortedDays.findIndex((d) => d.id === targetDayId);
      const insertionIndex = targetIndex === -1 ? sortedDays.length : targetIndex + 1; // insert after target

      await onCloneTrainingDay(sourceDayId, insertionIndex);
      return;
    }
  ...
  }, [trainingDays, exercises, getExercisesForDay, onCloneTrainingDay, onReorderExercises, onMoveExercise, onUpdateDayExercise]);
  ...
  return (
    ...
    >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    <span className="px-2 py-0.5 text-[11px] rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                      {t('trainingDay.warmup')}: {warmupCount}
                    </span>
                    <span className="px-2 py-0.5 text-[11px] rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                      {t('trainingDay.workout')}: {mainCount}
                    </span>
                    <span className="px-2 py-0.5 text-[11px] rounded-full bg-amber-50 text-amber-700 border border-amber-200">
                      {t('trainingDay.cooldown')}: {cooldownCount}
                    </span>
                    <span className="px-2 py-0.5 text-[11px] rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200">
                      {t('trainingDay.restDay')}: {restDayCount}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <DragOverlay modifiers={[snapCenterToCursor]} dropAnimation={null}>
        ...
        {activeItem?.type === 'training-day' && activeItem.trainingDay && (
          <div className="p-3 bg-white border-2 border-primary-500 rounded-lg shadow-xl cursor-grabbing min-w-[220px]">
            <div className="text-sm font-semibold text-gray-900">
              {t('trainingDay.dayName', { number: activeItem.trainingDay.day_number })}{activeItem.trainingDay.name ? `: ${activeItem.trainingDay.name}` : ''}
            </div>
            <div className="text-xs text-gray-600 mt-0.5">
              {t('trainingDay.exerciseCount', { count: activeItem.trainingDay.exercises?.length || 0 })}
            </div>
          </div>
        )}
      </DragOverlay>
