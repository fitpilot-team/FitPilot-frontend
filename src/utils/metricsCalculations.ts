...
export const RP_VOLUME_LANDMARKS: Record<MuscleName, VolumeLandmarks> = {
  ...
  lateral_deltoid: { mv: 0, mev: 6, mav: [12, 18], mrv: 22 },   // Side Delts need direct work
...
export function calculateVolumeByMuscleGroup(
  ...
  // ---------------------------------------------------------------------------
  // Deltoid spillover (fractional reps): anterior presses also hit medial/posterior,
  // and rear-delt work carries some fatigue to anterior. This improves heatmap nuance
  // without complicating the trainer view (which we can aggregate separately).
  // ---------------------------------------------------------------------------
  const deltoidSpillover: Record<string, { target: string; ratio: number }[]> = {
    // Presses/raises counted as anterior also tax lateral/posterior heads
    anterior_deltoid: [
      { target: 'lateral_deltoid', ratio: 0.4 },
      { target: 'posterior_deltoid', ratio: 0.2 },
    ],
    // Lateral work bleeds into both heads
    lateral_deltoid: [
      { target: 'anterior_deltoid', ratio: 0.2 },
      { target: 'posterior_deltoid', ratio: 0.2 },
    ],
    // Rear-delt rows/hits give a small carryover to anterior/lateral
    posterior_deltoid: [
      { target: 'anterior_deltoid', ratio: 0.15 },
      { target: 'lateral_deltoid', ratio: 0.25 },
    ],
  };

  Object.entries(deltoidSpillover).forEach(([source, targets]) => {
    const sourceData = volumeMap[source];
    if (!sourceData) return;
    targets.forEach(({ target, ratio }) => {
      if (!volumeMap[target]) {
        volumeMap[target] = { effectiveReps: 0, effectiveSets: 0, totalSets: 0, stressIndex: 0 };
      }
      volumeMap[target].effectiveReps += Math.round(sourceData.effectiveReps * ratio);
      volumeMap[target].effectiveSets += sourceData.effectiveSets * ratio;
      volumeMap[target].totalSets += sourceData.totalSets * ratio;
      volumeMap[target].stressIndex += sourceData.stressIndex * ratio;
    });
  });
...
export const VOLUME_THRESHOLDS: Record<MuscleName, { low: number; high: number }> = {
  ...
  lateral_deltoid: { low: 40, high: 80 },    // Requiere trabajo directo
