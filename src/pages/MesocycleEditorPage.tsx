...
import { MesocycleModal, MicrocycleModal, TrainingDayModal, CloneTrainingDaysModal } from './mesocycle-editor/modals';
...
const macrocycleSchema = z.object({
  ...
  // Optional: templates don't have a client
...
export function MesocycleEditorPage() {
  ...
  const [showCloneTrainingDaysModal, setShowCloneTrainingDaysModal] = useState(false);
  const [cloneDaysContext, setCloneDaysContext] = useState<{
    mesocycleId: string;
    microcycleId: string;
  } | null>(null);
  ...
  const {
    ...
    defaultValues: {
      status: 'draft',
      client_id: isTemplateRoute ? '' : selectedClient?.id || '',
  ...
  useEffect(() => {
    if (isNew && !isTemplateRoute && selectedClient?.id) {
      setValue('client_id', selectedClient.id);
    }
    if (isTemplateRoute) {
      setValue('client_id', '');
    }
  }, [selectedClient, isNew, setValue, isTemplateRoute]);
  ...
  const getNextExerciseOrderIndex = (microcycleId: string, dayId: string) => {
    const day = (trainingDays[microcycleId] || []).find((d) => d.id === dayId);
    const maxOrderIndex = (day?.exercises || []).reduce(
  ...
  const onSubmit = async (data: MacrocycleFormData) => {
    ...
    try {
      if (isNew) {
        const formClientId = (data.client_id || '').trim();
        const createData = {
          name: data.name,
          ...
          client_id: isTemplateRoute ? null : formClientId || null,
        };

        if (!isTemplateRoute && !createData.client_id) {
        ...
        if (isTemplateRoute) {
          navigate(`/templates/${macrocycle.id}`);
          return;
        }

        const targetClientId = selectedClient?.id || clientId;
        if (targetClientId) {
          navigate(`/clients/${targetClientId}/programs/${macrocycle.id}`);
          return;
        }
  ...
  const handleCloneMicrocycle = async (mesocycleId: string, microcycleId: string) => {
    if (!currentMacrocycle) return;

    const sourceMicrocycle = (microcycles[mesocycleId] || []).find((mc) => mc.id === microcycleId);
    if (!sourceMicrocycle) {
      toast.error('No se pudo encontrar el microciclo a clonar');
      return;
    }

    const existing = microcycles[mesocycleId] || [];
    const maxWeekNumber = existing.reduce((max, mc) => Math.max(max, mc.week_number), 0);
    const newWeekNumber = maxWeekNumber + 1;

    const sourceStart = parseISO(sourceMicrocycle.start_date);
    const sourceEnd = parseISO(sourceMicrocycle.end_date);
    const lengthDays = isValid(sourceStart) && isValid(sourceEnd)
      ? Math.max(1, differenceInCalendarDays(sourceEnd, sourceStart) + 1)
      : 7;

    const newStart = isValid(sourceStart) ? addDays(sourceStart, lengthDays) : addDays(new Date(), 7);
    const newEnd = isValid(sourceEnd) ? addDays(sourceEnd, lengthDays) : addDays(newStart, 6);

    const confirmMessage =
      `¿Clonar el microciclo "${sourceMicrocycle.name}"?\n\n` +
      `Se creará un nuevo microciclo (semana ${newWeekNumber}) con sus días y ejercicios.`;
    if (!confirm(confirmMessage)) return;

    const toastId = toast.loading('Clonando microciclo...');
    try {
      const clonedMicrocycle = await createMicrocycle(currentMacrocycle.id, mesocycleId, {
        week_number: newWeekNumber,
        name: `${sourceMicrocycle.name} (copia)`,
        start_date: format(newStart, 'yyyy-MM-dd'),
        end_date: format(newEnd, 'yyyy-MM-dd'),
        intensity_level: sourceMicrocycle.intensity_level,
        notes: sourceMicrocycle.notes || undefined,
      });

      const sourceDays = [...(trainingDays[sourceMicrocycle.id] || sourceMicrocycle.training_days || [])]
        .sort((a, b) => a.day_number - b.day_number);

      const offsetDays = isValid(sourceStart) ? differenceInCalendarDays(newStart, sourceStart) : lengthDays;

      for (const day of sourceDays) {
        const dayDate = isValid(parseISO(day.date))
          ? addDays(parseISO(day.date), offsetDays)
          : addDays(newStart, Math.max(0, day.day_number - 1));

        const clonedDay = await createTrainingDay(
          currentMacrocycle.id,
          mesocycleId,
          clonedMicrocycle.id,
          {
        ...
        for (let index = 0; index < dayExercises.length; index++) {
      ...
      await loadMacrocycle(currentMacrocycle.id);
      toast.success('Microciclo clonado correctamente');
    } catch (error: any) {
      toast.error(error.message || 'No se pudo clonar el microciclo');
    } finally {
      toast.dismiss(toastId);
    }
  };

  const handleCloneTrainingDays = async (dayIds: string[], insertionIndex: number) => {
    if (!currentMacrocycle || !cloneDaysContext) return;

    const { mesocycleId, microcycleId } = cloneDaysContext;
    const allDays = [...(trainingDays[microcycleId] || [])].sort((a, b) => a.day_number - b.day_number);
    const daysToClone = allDays.filter((d) => dayIds.includes(d.id));

    if (daysToClone.length === 0) {
      toast.error('Selecciona al menos un día para clonar');
      return;
    }

    const microcycle = (microcycles[mesocycleId] || []).find((mc) => mc.id === microcycleId);
    const microcycleStart = microcycle ? parseISO(microcycle.start_date) : new Date();
    const startDate = isValid(microcycleStart) ? microcycleStart : new Date();

    const safeInsertionIndex = Math.min(Math.max(0, insertionIndex), allDays.length);
    const cloneCount = daysToClone.length;

    const toastId = toast.loading('Clonando días...');
    try {
      // 1) Shift existing days to free day_number slots (descending to avoid collisions)
      const daysToShift = allDays.slice(safeInsertionIndex).sort((a, b) => b.day_number - a.day_number);
      for (const day of daysToShift) {
        const newDayNumber = day.day_number + cloneCount;
        await updateTrainingDay(currentMacrocycle.id, mesocycleId, microcycleId, day.id, {
          day_number: newDayNumber,
          date: format(addDays(startDate, Math.max(0, newDayNumber - 1)), 'yyyy-MM-dd'),
          name: day.name,
          focus: day.focus || '',
          rest_day: day.rest_day,
          notes: day.notes || '',
      ...
      // 2) Create cloned days + exercises at insertion position
      for (let index = 0; index < daysToClone.length; index++) {
        const sourceDay = daysToClone[index];
        const newDayNumber = safeInsertionIndex + index + 1;

        const clonedDay = await createTrainingDay(currentMacrocycle.id, mesocycleId, microcycleId, {
          day_number: newDayNumber,
          name: sourceDay.name,
          date: format(addDays(startDate, Math.max(0, newDayNumber - 1)), 'yyyy-MM-dd'),
          focus: sourceDay.focus || '',
          rest_day: sourceDay.rest_day,
          notes: sourceDay.notes || '',
        });

        const dayExercises = [...(sourceDay.exercises || [])].sort((a, b) => a.order_index - b.order_index);
        for (let exIndex = 0; exIndex < dayExercises.length; exIndex++) {
          const exercise = dayExercises[exIndex];
          await createDayExercise(currentMacrocycle.id, mesocycleId, microcycleId, clonedDay.id, {
            exercise_id: exercise.exercise_id,
            order_index: exIndex,
            phase: exercise.phase,
            sets: exercise.sets,
      ...
      await loadMacrocycle(currentMacrocycle.id);
      toast.success('Días clonados correctamente');
      setShowCloneTrainingDaysModal(false);
      setCloneDaysContext(null);
    } catch (error: any) {
      toast.error(error.message || 'No se pudieron clonar los días');
    } finally {
      toast.dismiss(toastId);
    }
  };

  const handleCloneTrainingDay = async (microcycleId: string, sourceDayId: string, insertionIndex: number) => {
    if (!currentMacrocycle) return;
    ...
    const microcycle = (microcycles[foundMesocycleId] || []).find((mc) => mc.id === microcycleId);
    const microcycleStart = microcycle ? parseISO(microcycle.start_date) : new Date();
    const startDate = isValid(microcycleStart) ? microcycleStart : new Date();

    const safeInsertionIndex = Math.min(Math.max(0, insertionIndex), allDays.length);
    const toastId = toast.loading('Clonando d\u00EDa...');

    // Snapshot source day before any shifts happen
    const sourceDaySnapshot = {
      name: sourceDay.name,
      focus: sourceDay.focus || '',
      rest_day: sourceDay.rest_day,
      notes: sourceDay.notes || '',
      exercises: [...(sourceDay.exercises || [])].sort((a, b) => a.order_index - b.order_index),
    };

    try {
      // 1) Shift existing days to free the day_number slot
      const daysToShift = allDays
        .slice(safeInsertionIndex)
        .sort((a, b) => b.day_number - a.day_number);
      for (const day of daysToShift) {
        const newDayNumber = day.day_number + 1;
        await updateTrainingDay(currentMacrocycle.id, foundMesocycleId, microcycleId, day.id, {
          day_number: newDayNumber,
          date: format(addDays(startDate, Math.max(0, newDayNumber - 1)), 'yyyy-MM-dd'),
          name: day.name,
          focus: day.focus || '',
      ...
      // 2) Create the cloned day at insertion position
      const newDayNumber = safeInsertionIndex + 1;
      const clonedDay = await createTrainingDay(currentMacrocycle.id, foundMesocycleId, microcycleId, {
        day_number: newDayNumber,
        name: sourceDaySnapshot.name,
        date: format(addDays(startDate, Math.max(0, newDayNumber - 1)), 'yyyy-MM-dd'),
        focus: sourceDaySnapshot.focus,
        rest_day: sourceDaySnapshot.rest_day,
        notes: sourceDaySnapshot.notes,
      });

      // 3) Clone exercises
      for (let exIndex = 0; exIndex < sourceDaySnapshot.exercises.length; exIndex++) {
        const exercise = sourceDaySnapshot.exercises[exIndex];
        await createDayExercise(currentMacrocycle.id, foundMesocycleId, microcycleId, clonedDay.id, {
          exercise_id: exercise.exercise_id,
          order_index: exIndex,
          phase: exercise.phase,
          sets: exercise.sets,
      ...
      await loadMacrocycle(currentMacrocycle.id);
      toast.success('D\u00EDa clonado correctamente');
    } catch (error: any) {
      toast.error(error.message || 'No se pudo clonar el d\u00EDa');
    } finally {
      toast.dismiss(toastId);
    }
  };
    ...
    try {
      await createDayExercise(
        ...
        selectedContext.dayId,
        {
          ...
          notes: config.notes,
          order_index: getNextExerciseOrderIndex(selectedContext.microcycleId, selectedContext.dayId),
        }
      );
  ...
  const openCloneTrainingDays = (mesocycleId: string, microcycleId: string) => {
    setCloneDaysContext({ mesocycleId, microcycleId });
    setShowCloneTrainingDaysModal(true);
  };
  ...
  const handleKanbanCreateDayExercise = async (dayId: string, exerciseId: string, config: ExerciseConfigData) => {
    ...
    try {
      ...
      const isCardio = exercise?.exercise_class === 'cardio';
      const isHiit = isCardio && exercise?.cardio_subclass === 'hiit';
      ...
      await createDayExercise(
        ...
        dayId,
        {
          ...
          set_type: config.set_type,
          notes: config.notes,
          order_index: getNextExerciseOrderIndex(kanbanContext.microcycleId, dayId),
          ...(isCardio
            ? {
                duration_seconds: config.duration_seconds ?? null,
                intensity_zone: config.intensity_zone ?? null,
                distance_meters: config.distance_meters ?? null,
                target_calories: config.target_calories ?? null,
              }
            : {}),
          ...(isHiit
            ? {
                intervals: config.intervals ?? null,
                work_seconds: config.work_seconds ?? null,
                interval_rest_seconds: config.interval_rest_seconds ?? null,
              }
            : {}),
        }
      );
  ...
  const handleKanbanUpdateDayExercise = async (dayExerciseId: string, config: ExerciseConfigData) => {
    ...
    let targetDayExercise: any = null;
    for (const day of days) {
      const foundExercise = day.exercises?.find((e) => e.id === dayExerciseId);
      if (foundExercise) {
        targetDayId = day.id;
        targetDayExercise = foundExercise;
        break;
      }
    }
    ...
    try {
      const exercise =
        targetDayExercise?.exercise ||
        exercises.find((e) => e.id === targetDayExercise?.exercise_id);
      const isCardio = exercise?.exercise_class === 'cardio';
      const isHiit = isCardio && exercise?.cardio_subclass === 'hiit';
  ...
  const handleEmbeddedKanbanCreateDayExercise = async (
    ...
    try {
      ...
      await createDayExercise(currentMacrocycle.id, foundMesocycleId, microcycleId, dayId, {
        ...
        set_type: config.set_type,
        notes: config.notes,
        order_index: getNextExerciseOrderIndex(microcycleId, dayId),
        ...(isCardio
          ? {
              duration_seconds: config.duration_seconds ?? null,
              intensity_zone: config.intensity_zone ?? null,
  ...
  return (
    <>
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <Button
              ...
              onClick={() => {
                const targetClientId = selectedClient?.id || clientId;
                if (targetClientId) {
                  navigate(`/clients/${targetClientId}/programs`);
          ...
          <div>
            ...
            {currentMesocycles.length === 0 ? (
              ...
              <DndWrapper
                ...
                onCloneTrainingDay={handleCloneTrainingDay}
              >
                  ...
                  {currentMesocycles
                    ...
                    .map((mesocycle) => (
                      <MesocycleCard
                        ...
                        onCloneMicrocycle={(microcycleId) => handleCloneMicrocycle(mesocycle.id, microcycleId)}
                        onCloneTrainingDays={(microcycleId) => openCloneTrainingDays(mesocycle.id, microcycleId)}
                        onCloneTrainingDay={handleCloneTrainingDay}
        ...
        {/* Clone Training Days Modal */}
        <CloneTrainingDaysModal
          isOpen={showCloneTrainingDaysModal}
          trainingDays={cloneDaysContext ? (trainingDays[cloneDaysContext.microcycleId] || []) : []}
          onClose={() => {
            setShowCloneTrainingDaysModal(false);
            setCloneDaysContext(null);
          }}
          onSubmit={handleCloneTrainingDays}
        />
