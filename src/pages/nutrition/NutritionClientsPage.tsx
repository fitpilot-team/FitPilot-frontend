...
export function NutritionClientsPage() {
  ...
  } = useGetAppointments(professionalId);
  const clientCount = rawClients?.length ?? 0;
  const hasClientLimit = planAccess.maxClients !== null;
  const hasReachedClientLimit =
    hasClientLimit && clientCount >= Number(planAccess.maxClients);
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-900">
                  Tu plan Starter permite hasta {planAccess.maxClients}{" "}
                  clientes.
                </p>
                <p className="text-sm text-amber-800">
                  Para agregar más clientes, mejora tu plan desde la sección de
                  suscripciones.
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedClients.map((client) => (
            <ClientCard
              ...
              onIntakeAction={() =>
                navigate(`/nutrition/clients/${client.id}/intake`)
              }
              onAction={() =>
                navigate(`/nutrition/clients/${client.id}/medical-history`)
              }
            />
          ))}
        </div>
