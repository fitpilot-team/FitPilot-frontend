...
import {
  ...
  useUpdateAppointmentDraft,
  useGetAppointmentDraft,
...
const normalizeTrainingProfile = (raw: any): TrainingProfileState => {
  ...
  return {
    ...
    exercise_variety:
      raw.exercise_variety || DEFAULT_TRAINING_PROFILE.exercise_variety,
    include_cardio:
      raw.include_cardio ?? DEFAULT_TRAINING_PROFILE.include_cardio,
    include_warmup:
      raw.include_warmup ?? DEFAULT_TRAINING_PROFILE.include_warmup,
    include_cooldown:
      raw.include_cooldown ?? DEFAULT_TRAINING_PROFILE.include_cooldown,
  };
};
...
const buildTrainingInterviewPayload = (
  ...
  target_muscle_groups: profile.target_muscle_groups?.length
  ...
  preferred_days: profile.preferred_days?.length
    ? profile.preferred_days
    : undefined,
  has_gym_access: profile.has_gym_access,
  available_equipment: profile.available_equipment?.length
  ...
  excluded_exercises: profile.excluded_exercises?.length
    ? profile.excluded_exercises
    : undefined,
  medical_conditions: profile.medical_conditions?.length
  ...
  preferred_training_style:
    profile.preferred_training_style?.trim() || undefined,
...
export function NutritionConsultationPage() {
  ...
  const clientAppointments = appointmentsList.filter(
    (a) => Number(a.client_id) === Number(appointment?.client_id),
  );
  ...
  const pastAppointments = clientAppointments
    .filter(
      (a) => a.id !== appointment?.id && new Date(a.scheduled_at) < new Date(),
    )
    .sort(
      (a, b) =>
  ...
  const [activeSection, setActiveSection] = useState<
    keyof typeof noteSections | null
  >(null);
  ...
  const [view, setView] = useState<
    "notes" | "progress" | "metrics" | "planning"
  >("notes");
  ...
  const [trainingProfile, setTrainingProfile] = useState<TrainingProfileState>(
    () => ({ ...DEFAULT_TRAINING_PROFILE }),
  );
  const [trainingProfileDirty, setTrainingProfileDirty] = useState(false);
  const [isLoadingTrainingProfile, setIsLoadingTrainingProfile] =
    useState(false);
  const [isSyncingTrainingProfile, setIsSyncingTrainingProfile] =
    useState(false);
  const [trainingSyncWarning, setTrainingSyncWarning] = useState<string | null>(
    null,
  );
  ...
  const handleSaveMetrics = () => {
    ...
    const processedMetrics = Object.entries(metrics).reduce(
      (acc, [key, value]) => {
    ...
    const metricData = {
      ...
      ...(professionalId
        ? { recorded_by_user_id: Number(professionalId) }
        : {}),
      ...processedMetrics,
    };
  ...
  const syncTrainingProfileToBackend = async (
    force = false,
  ): Promise<boolean> => {
  ...
  const handleMacroKeyDown = (
    e: React.KeyboardEvent,
    type: "proteins" | "carbs" | "fats",
  ) => {
    if (e.key === "ArrowUp") {
      e.preventDefault();
      updateMacro(type, 5);
  ...
  useEffect(() => {
    ...
    if (isAnotherSessionActive) {
      setWarningMessage(
        "Hay otra sesión en curso. Paúsala o finalízala antes de iniciar esta.",
      );
    ...
    startMutation.mutate(appointment.id, {
      onSuccess: (updatedAppointment) => {
        ...
        sessionEventsRef.current = Array.isArray(
  ...
  const appointmentId = Number.isFinite(parsedAppointmentId)
    ? parsedAppointmentId
    : 0;
  const { data: existingDraft, isLoading: isLoadingDraft } =
    useGetAppointmentDraft(appointmentId);
      ...
      if (existingDraft.json_state) {
        ...
        if (existingDraft.notes) {
          ...
          parts.forEach((part: string) => {
            if (part.startsWith("MOTIVO DE LA CONSULTA:"))
              sections.motivo = part.replace("MOTIVO DE LA CONSULTA:\n", "");
            else if (part.startsWith("EVOLUCIÓN DESDE LA ÚLTIMA SESIÓN:"))
              sections.evolucion = part.replace(
                "EVOLUCIÓN DESDE LA ÚLTIMA SESIÓN:\n",
                "",
              );
            else if (part.startsWith("INDICACIONES DADAS:"))
              sections.indicaciones = part.replace("INDICACIONES DADAS:\n", "");
            else if (part.startsWith("ACUERDOS / PRÓXIMOS PASOS:"))
              sections.acuerdos = part.replace(
        ...
        if (existingDraft.metrics) {
          const metricsSource =
            existingDraft.metrics.metric || existingDraft.metrics;
          const loadedMetrics = { ...metrics };
          Object.keys(metricsSource).forEach((key) => {
            if (
              metricsSource[key] !== null &&
              metricsSource[key] !== undefined
            ) {
              loadedMetrics[key as keyof typeof metrics] = String(
                metricsSource[key],
              );
            }
          });
    ...
    if (
      ...
      if (appointment.status === "paused" && appointment.paused_at) {
        const pauseTime = Math.floor(
          (now.getTime() - new Date(appointment.paused_at).getTime()) / 1000,
        );
  ...
  const saveDraft = () => {
    ...
    const processedMetrics = Object.entries(state.metrics).reduce(
      (acc, [key, value]) => {
        acc[key] = value === "" ? null : Number(value);
        return acc;
      },
  ...
  const handleToggleTimer = () => {
    ...
    if (isAnotherSessionActive) {
      setWarningMessage(
        "Hay otra sesión en curso. Paúsala o finalízala antes de reanudar esta.",
      );
      setShowWarningToast(true);
      return;
    }
      ...
      if (isActive) {
        if (appointment?.id) {
          ...
          try {
            ...
            fetch(
              `${import.meta.env.VITE_NUTRITION_API_URL}/v1/appointments/${appointment.id}`,
              {
                method: "PATCH",
                headers,
                keepalive: true,
    ...
    return () => {
      ...
      if (state.isActive && state.appointmentId) {
        try {
          ...
          fetch(
            `${import.meta.env.VITE_NUTRITION_API_URL}/v1/appointments/${state.appointmentId}`,
            {
              method: "PATCH",
              headers,
              keepalive: true,
  ...
  useEffect(() => {
    ...
    const pausedSeconds = initialPausedAt
      ? Math.max(
          ...
          Math.floor(
            (new Date(resumedAt).getTime() -
              new Date(initialPausedAt).getTime()) /
              1000,
          ),
        )
      : 0;
  ...
  const isInitialDataLoading =
    ...
    (Boolean(professionalId) &&
      ((!hasAppointmentsData && isLoadingAppointments) ||
        (!hasClientsData && isLoadingClients)));
        ...
        <div className="w-full mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="relative group/timer">
              ...
              >
                <Clock
                  className={`w-4 h-4 ${isActive ? "text-nutrition-500 animate-pulse" : "text-gray-400"}`}
                />
                <span
                  className={`font-mono font-bold tracking-tight text-gray-700 tabular-nums ${isTimerExpanded ? "text-sm" : "text-xs"}`}
                >
                  {formatTime(seconds)}
                </span>
          ...
          <div className="lg:col-span-3 flex flex-col gap-6 h-[calc(100vh-140px)] sticky top-24 overflow-y-auto custom-scrollbar pr-2 pb-4">
            ...
            <PatientSnapshotCard
              client={{
                ...
                medical_conditions: hasConditions
                  ? medicalConditions
                  : undefined,
                gender:
                  clientHistory?.genre ||
            ...
            <ConsultationStepper
              currentStep={
                view as "notes" | "progress" | "metrics" | "planning"
              }
              onStepChange={(
                step: "notes" | "progress" | "metrics" | "planning",
              ) => setView(step)}
              progress={{
                measurements:
                  completedSteps.measurements ||
                  !!(metrics.weight || metrics.height),
                progress: completedSteps.progress,
                notes:
                  completedSteps.notes ||
                  Object.values(noteSections).some((val) => val.length > 5),
            ...
            {view === "notes" && (
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-6 max-h-[65vh] overflow-y-auto pr-2 custom-scrollbar">
                  {(
                    ["motivo", "evolucion", "indicaciones", "acuerdos"] as const
                  ).map((section) => {
                    const labels = {
                      motivo: "Motivo de la consulta",
                      evolucion: "Evolución desde la última sesión",
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Ingresa los datos antropométricos del paciente
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, body_fat_pct: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, waist_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, hip_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, visceral_fat: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, water_pct: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, chest_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.cephalic_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, cephalic_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.neck_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, neck_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.mesosternal_cm}
                      onChange={(e) =>
                        setMetrics({
                          ...metrics,
                          mesosternal_cm: e.target.value,
                        })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.umbilical_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, umbilical_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                ...
                <div className="mb-4">
                  <h4 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4">
                    Perímetros Complementarios
                  </h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 ml-1">
                      Mitad del Brazo Relajado (cm)
                    </label>
                    <input
                      type="number"
                      value={metrics.relaxed_arm_midpoint_cm}
                      onChange={(e) =>
                        setMetrics({
                          ...metrics,
                          relaxed_arm_midpoint_cm: e.target.value,
                        })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 ml-1">
                      Mitad del Brazo Contraído (cm)
                    </label>
                    <input
                      type="number"
                      value={metrics.contracted_arm_midpoint_cm}
                      onChange={(e) =>
                        setMetrics({
                          ...metrics,
                          contracted_arm_midpoint_cm: e.target.value,
                        })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.forearm_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, forearm_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.wrist_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, wrist_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.mid_thigh_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, mid_thigh_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="number"
                      value={metrics.calf_cm}
                      onChange={(e) =>
                        setMetrics({ ...metrics, calf_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, arm_left_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, arm_right_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetrics({ ...metrics, calf_left_cm: e.target.value })
                      }
                      className="w-full p-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-blue-200 focus:bg-white outline-none transition-all font-bold text-gray-900"
                      placeholder="0.0"
                    />
            ...
            {view === "planning" && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  ...
                  />
                  <div>
                    <label className="text-[9px] font-bold text-gray-400 ml-1 uppercase block mb-1">
                      Comidas al Día
                    </label>
                    <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl p-2 h-[42px]">
                      <button
                        onClick={() =>
                          setMealsPerDay((prev) => Math.max(1, prev - 1))
                        }
                      ...
                      <button
                        onClick={() =>
                          setMealsPerDay((prev) => Math.min(8, prev + 1))
                        }
                        className="w-8 h-full flex items-center justify-center text-gray-400 hover:text-nutrition-600 hover:bg-nutrition-50 rounded-lg transition-colors"
                      >
                        +
                  ...
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between mb-4">
                    <div>
                      <h4 className="text-lg font-bold text-blue-900">
                        Contexto IA Training
                      </h4>
                      <p className="text-sm text-blue-700">
                        Se guarda automáticamente en borrador y se sincroniza
                        con Training.
                      </p>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateTrainingProfileField("phone", e.target.value)
                        }
                        className="w-full p-3 rounded-xl border border-blue-100 bg-white text-sm"
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateTrainingProfileField("address", e.target.value)
                      }
                      className="w-full p-3 rounded-xl border border-blue-100 bg-white text-sm"
                      rows={2}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <input
                        type="text"
                        value={joinArrayForInput(
                          trainingProfile.target_muscle_groups as any,
                        )}
                        onChange={(e) =>
                          updateTrainingProfileField(
                            "target_muscle_groups",
                            splitCsvToStringArray(e.target.value) as any,
                          )
                        }
                        className="w-full p-3 rounded-xl border border-blue-100 bg-white text-sm"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <input
                        type="text"
                        value={joinArrayForInput(
                          trainingProfile.preferred_days,
                        )}
                        onChange={(e) =>
                          updateTrainingProfileField(
                            "preferred_days",
                            splitCsvToDayArray(e.target.value),
                          )
                        }
                        className="w-full p-3 rounded-xl border border-blue-100 bg-white text-sm"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <input
                        type="text"
                        value={joinArrayForInput(
                          trainingProfile.available_equipment as any,
                        )}
                        onChange={(e) =>
                          updateTrainingProfileField(
                            "available_equipment",
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <input
                        type="text"
                        value={joinArrayForInput(
                          trainingProfile.injury_areas as any,
                        )}
                        onChange={(e) =>
                          updateTrainingProfileField(
                            "injury_areas",
                            splitCsvToStringArray(e.target.value),
                          )
                        }
                        className="w-full p-3 rounded-xl border border-blue-100 bg-white text-sm"
                      ...
                      <input
                        type="text"
                        value={joinArrayForInput(
                          trainingProfile.excluded_exercises,
                        )}
                        onChange={(e) =>
                          updateTrainingProfileField(
                            "excluded_exercises",
                      ...
                      <input
                        type="text"
                        value={joinArrayForInput(
                          trainingProfile.medical_conditions,
                        )}
                        onChange={(e) =>
                          updateTrainingProfileField(
                            "medical_conditions",
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 rounded-3xl bg-gray-50 border border-transparent hover:border-gray-200 focus-within:border-nutrition-400 focus-within:ring-2 focus-within:ring-nutrition-100 transition-all group">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-baseline gap-0.5">
                        <input
                          ...
                          onChange={(e) =>
                            handleMacroInputChange(e, "proteins")
                          }
                      ...
                      <div className="flex gap-1">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            updateMacro("proteins", -5);
                          }}
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-gray-200 text-gray-400 hover:text-nutrition-600 hover:border-nutrition-200 transition-all disabled:opacity-50 active:scale-95 text-lg font-bold"
                        >
                          -
                        </button>
                        <button
                ...
                <div className="mt-4 p-4 rounded-3xl bg-nutrition-50 border border-nutrition-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="font-bold text-nutrition-800">
                        Objetivo Calórico
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-nutrition-700">
                      ...
                      {activeGoal?.goals && (
                        ...
                        >
                          ...
                          <span>
                            ...
                            {activeGoal.goals.adjustment_type === "percent"
                              ? "%"
                              : " kcal"}
                          </span>
                        </div>
                    ...
                    {activeGoal?.goals ? (
                      <>
                        <div className="flex justify-between items-center">
                          <span>
                            Ajuste por Objetivo ({activeGoal.goals.name}):
                          </span>
                          <span
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-gray-500 text-sm mb-8 leading-relaxed">
                Asegúrate de haber guardado todos los ajustes importantes antes
                de salir.
              </p>
            ...
            {modalTab === "history" ? (
              pastAppointments.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-400">
                    Esta es la primera sesión registrada
                  </p>
                </div>
              )
              ...
              <div className="space-y-4">
                {upcomingAppointments.map((apt) => (
                  <div
                    key={apt.id}
                    className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100 flex items-center justify-between"
                  >
                    <div>
                      <p className="font-bold text-gray-900">
                        {format(
              ...
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-400">
                  Agenda una nueva cita para verla aquí
                </p>
              </div>
            )}
          </div>
