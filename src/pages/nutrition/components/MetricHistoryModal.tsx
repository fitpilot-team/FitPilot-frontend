...
export function MetricHistoryModal({
  ...
  useEffect(() => {
    setActiveTab(tabs ? tabs[0].metricType : null);
  }, [tabs, isOpen]);
  ...
  if (series) {
    ...
    processedData = Array.from(metricsByDate.values()).sort(
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {tabs.map((tab) => (
                      <button
                        key={tab.metricType}
                        type="button"
                        onClick={() => setActiveTab(tab.metricType)}
                        className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
                          activeTab === tab.metricType
